...
def make_movie_all(numElem, chainSize, steps, runs):
    zoom = 0.3
    x_middle = y_middle = z_middle = plot_radius = 0.0
    filename_movie = "../res/dipVecsCoords_all_c"+str(chainSize)+"_e"+str(numElem)+"_r"+str(runs)+"_s"+str(steps)
    filename = "../res/snaps/frame_"+"_c"+str(chainSize)+"_e"+str(numElem)+"_r"+str(runs)+"_s"+str(steps)+"_z"+str(zoom)
    ...
    with open(filename_movie+".txt") as f:
        while step < steps:
            ...
            if line[:4] == '#run':
                ...
                ax.set_zlabel('z')
                if step == 1:
                    x_middle, y_middle, z_middle, plot_radius = set_axes_equal(ax, zoom=zoom)
                else:
                    ax.set_xlim3d([x_middle - plot_radius, x_middle + plot_radius])
                    ax.set_ylim3d([y_middle - plot_radius, y_middle + plot_radius])
                    ax.set_zlim3d([z_middle - plot_radius, z_middle + plot_radius])
                ...
                fig.savefig(filename+"_%03d.png"%(step-1), dpi=300)
                plt.close(fig)
