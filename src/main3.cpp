...
int main()
{
    ...
    //Mersenne Twister:
    random_device rng;
    uniform_real_distribution<double> randVec(-1.0/sqrt(3), 1.0/sqrt(3));
    ...
    dataFileIO.snapshot(chain, "first", startenergy, 0.0, 0.0, r_+ edgeLength_);
     ...
     * 
     */
     //----------------------------------mc steps for magnetic chain----------------------------
     cout << "mc steps for magnetic chain <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" << endl;
    //start_c = clock();
    time(&start_t);
    ...
    for(int i = 0; i < runs_; i++)
    {
        if(i%(runs_/steps_) == 0)
        {
            ...
            cube cy = chain.rotateSelf(Cube, rotVec, rotPhi);
            //cout << "cy: " << cy << endl;
            chain.cubeArray[Cube] = cy;
    ...
    dataFileIO.snapshot(chain, "last", energy, angleCubePos/(double)steps_, angleMagnMom/(double)steps_, avgCenterDist);
